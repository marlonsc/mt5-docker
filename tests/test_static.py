...
class TestVersionsEnv:
    ...
    def test_python_version_is_312(self) -> None:
        ...
        assert version.startswith(c.VersionCheck.Prefix.PYTHON), (
            f"Python must be {c.VersionCheck.Prefix.PYTHON}.x, got {version}"
        )
    ...
    def test_numpy_version_is_126x(self) -> None:
        ...
        version = match.group(1)
        assert version.startswith(c.VersionCheck.Prefix.NUMPY), (
            f"NumPy must be {c.VersionCheck.Prefix.NUMPY}.x for Wine, got {version}"
        )
