...
if ! declare -f log > /dev/null 2>&1; then
    log() {
        local level="$1"; shift
        printf '[%s] [%s] %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ || true)" "${level}" "$*"
    }
fi
...
init_wine_prefix() {
    ...
    mono_msi=$(find "${staging}" -maxdepth 1 -name 'wine-mono-*-x86.msi' 2>/dev/null | head -1) || true
    gecko64_msi=$(find "${staging}" -maxdepth 1 -name 'wine-gecko-*-x86_64.msi' 2>/dev/null | head -1) || true
    gecko32_msi=$(find "${staging}" -maxdepth 1 -name 'wine-gecko-*-x86.msi' 2>/dev/null | head -1) || true
